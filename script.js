...
async function loadTree() {
  try {
    ...
    if (doc.exists) {
      treeData = doc.data().data || {};  // récupère treeData depuis la propriété "data"
    } else {
      treeData = {};
    }
  } catch (e) {
...
function saveTree() {
  dbRef.set({ data: treeData })  // on met treeData dans une propriété "data"
    .catch(e => console.error("Erreur sauvegarde Firestore:", e));
}
